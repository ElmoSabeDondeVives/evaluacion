...
$("#formulario_img").on('submit', function(e){
    e.preventDefault()
    var valor = true;
    var boton = "btn-agregar-imagen";
    var id_producto = $('#id_producto').val();
    var id_imgproductos = $('#id_imgproductos').val();
    var imagen = $('#imagen').val();
    valor = validar_campo_vacio('imagen',imagen, valor);
    if(valor){
        $.ajax({
            type: "POST",
            url: urlweb + "api/Producto/guardar_productoimg",
            ...
            success:function (r) {
                ...
                switch (r.result.code) {
                    case 1:
                        respuesta('¡Imagen guardada! Recargando...', 'success');
                        setTimeout(function () {
                            location.reload();
                        }, 1000);
                        break;
                    case 2:
                        respuesta('Error al guardar imagen', 'error');
                        break;
                    case 3:
                        respuesta('La imagen ya se encuentra registrada', 'error');
                        break;
                    default:
...
function editarproducto(id_producto){
    ...
    let guardarid = id_producto;
        $.ajax({
            ...
            // $('#producto_imagen').val(almacenar.producto_imagen);
            $('#producto_estado').val(almacenar.producto_estado);
            $('#visualizar').attr('src',urlweb+almacenar.producto_imagen);
        });
function edicion(producto_nombre, producto_precio, producto_stock, producto_descuento, producto_precioantiguo, producto_marca, producto_reseña, producto_caracteristicas, producto_detalle, producto_valoracion, producto_imagen, producto_estado){

}

}

// document.getElementById("producto_imagen").onchange = function(e){
...
//Funcion para previsualizar la imagen que se carga antes de guardar
...
function eliminar(id_imgproductos){
    $.ajax({
        type: "POST",
        url: urlweb + "api/Producto/eliminar",
        data: {
            id : id_imgproductos
        },
        dataType: 'json',
        success:function (r) {
            switch (r.result.code) {
                case 1:
                    respuesta('¡Imagen eliminada! Recargando...', 'success');
                    setTimeout(function () {
                        location.reload();
                    }, 1000);
                    break;
                case 2:
                    respuesta('Error al borrar imagen', 'error');
                    break;
                default:
                    respuesta('¡Algo catastrofico ha ocurrido!', 'error');
                    break;

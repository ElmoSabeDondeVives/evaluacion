...
function gestionar_subcategoria(){
    var valor = true;
    //Definimos el botón que activa la función
    var boton = "btn-agregar-subcategoria";
    //Extraemos las variable según los valores del campo consultado
    var id_categoria = $('#id_categoria').val();
    var id_subcategoria = $('#id_subcategoria').val();
    var subcategoria_nombre = $('#subcategoria_nombre').val();
    var subcategoria_estado = $('#subcategoria_estado').val();
    //Validamos si los campos a usar no se encuentran vacios
    valor = validar_campo_vacio('subcategoria_nombre', subcategoria_nombre, valor);
    valor = validar_campo_vacio('subcategoria_estado', subcategoria_estado, valor);
    ...
    if(valor){
        ...
        var cadena = "subcategoria_nombre=" + subcategoria_nombre +
            "&subcategoria_estado=" + subcategoria_estado+
            "&id_categoria=" + id_categoria+
            "&id_subcategoria=" + id_subcategoria;
        $.ajax({
            type: "POST",
            url: urlweb + "api/Subcategoria/guardar_subcategoria",
            data: cadena,
            dataType: 'json',
            beforeSend: function () {
            ...
            success:function (r) {
                ...
                switch (r.result.code) {
                    case 1:
                        if(id_subcategoria != ""){
                            respuesta('¡Subcategoria Editado Exitosamente', 'success');
                        } else {
                            respuesta('¡Subcategoria guardado! Recargando...', 'success');
                        }
                        setTimeout(function () { location.reload(); }, 1000);
                        break;
                    case 2:
                        respuesta('Error al guardar subcategoria', 'error');
                        break;
                    case 3:
                        respuesta('La subcategoria ya se encuentra registrada', 'error');
                        break;
                    default:
...
function editarsubcategoria(id_subcategoria){
    ...
    let guardarid = id_subcategoria;
    $.ajax({
        type: "POST",
        url: urlweb + "api/Subcategoria/mostrar",
        data: {
            guardarid :guardarid
        },
        dataType: 'json'
        ...
        $("#id_subcategoria").val(guardarid);
        $("#subcategoria_nombre").val(almacenar.subcategoria_nombre);
        $("#subcategoria_estado").val(almacenar.subcategoria_estado);
    });
    function edicion(subcategoria_nombre, subcategoria_estado){

...
class CategoriaController{
    ...
    public function inicio(){
        try{
            ...
            $nombre = $this->categoria->listarxnombre($id_familia);
            //Hacemos el require de los archivos a usar en las vistas
            require _VIEW_PATH_ . 'header.php';
            require _VIEW_PATH_ . 'navbar.php';
            require _VIEW_PATH_ . 'categoria/inicio.php';
	...
	public function guardar_categoria(){
		...
		try{
			$ok_data = true;
			$ok_data = $this->validar->validar_parametro('categoria_nombre', 'POST',true,$ok_data,50,'texto',0);
			$ok_data = $this->validar->validar_parametro('categoria_estado', 'POST',true,$ok_data,11,'numero',0);
			if($ok_data){
				$id = $_POST['id_categoria'];
				if($id == null){
					$validar_duplicados = $this->categoria->buscar_categoria($_POST['categoria_nombre']);
					if($validar_duplicados){
						$result = 3;
						$message = "Ya existe una categoria con ese nombre";
					}else{
						$result=$this->builder->save("categorias", array(
					...
					$validar_nombre = $this->categoria->buscar_categoria($_POST['categoria_nombre']);
					$validar_nombre_id = $this->categoria->buscar_categoria_id($_POST['categoria_nombre'],$_POST['id_categoria']);
					if($validar_nombre->id_categoria != $validar_nombre_id->id_categoria){
						$result = 3;
						$message = "Ya existe una categoria con ese nombre";
					}else{
						$result=$this->builder->update("categorias", array(
							"id_familia" => $_POST['id_familia'],
							"categoria_nombre" => $_POST['categoria_nombre'],
							"categoria_estado" => $_POST['categoria_estado']
						), array(
		...
		//Retornamos el json
		echo json_encode(array("result" => array("code" => $result, "message" => $message,"id_familia"=>$id)));
	}
    //Funcion para guardar un nuevo menú creado
	...
	public function mostrar(){
		...
		try{
			...
			if($ok_data){
				...
				$result = $this->categoria->listarxid($dato);
				//Validamos la duplicidad del $_POST['menu_controlador'], para evitar duplicados
			} else {
				//Código 6: Integridad de datos erronea
				$result = 6;
